...
test.describe('Sempoa Board - UI Layout & Responsive Design', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Sempoa Board' }),
    ).toBeVisible();
  });

  test.describe('Column Header Alignment', () => {
    test('should verify column headers are centered above their respective bead columns', async ({
      page,
    }) => {
      // Get column headers and bead columns
      const columnHeaders = page.locator('.column-header');
      const boardContainer = page
        .locator('.bg-amber-100.rounded.border-2.border-amber-800')
    ...
    test('should display place values in readable format within limited space', async ({
      page,
    }) => {
      const columnHeaders = page.locator('.column-header');
      ...
      for (let i = 0; i < SEMPOA_CONFIG.COLUMNS; i++) {
        ...
        const containsAbbreviation =
          (expectedValue === '1,000,000,000,000' &&
            headerText?.includes('1T')) ||
          (expectedValue === '100,000,000,000' &&
            headerText?.includes('100B')) ||
    ...
    test('should not interfere with bead interaction areas', async ({
      page,
    }) => {
      ...
      await expect(columnHeaders).toHaveCount(SEMPOA_CONFIG.COLUMNS);
      await expect(allBeads).toHaveCount(
          ...
          for (let j = 0; j < totalBeads; j++) {
            ...
            if (beadBox) {
              ...
              const hasVerticalOverlap = !(
                headerBox.y + headerBox.height < beadBox.y ||
              ...
              if (hasVerticalOverlap) {
                ...
                const hasHorizontalOverlap = !(
                  headerBox.x + headerBox.width < beadBox.x ||
  ...
  test.describe('Horizontal Separator Layout', () => {
    test('should span full board width and maintain proper alignment', async ({
      page,
    }) => {
      ...
      expect(separatorBox!.x + separatorBox!.width).toBeGreaterThanOrEqual(
        expectedRightEdge - tolerance,
      );
      ...
      expect(Math.abs(separatorCenter - boardCenter)).toBeLessThanOrEqual(
        centerTolerance,
      );
    });

    test('should capture visual alignment for manual verification', async ({
      page,
    }) => {
      await page.waitForSelector('[data-testid="sempoa-board"]');
  ...
  test.describe('Responsive Layout Behavior', () => {
    ...
    test('should maintain header alignment across viewport sizes', async ({
      page,
    }) => {
    ...
    for (const viewport of [
      ...
      test(`should maintain separator width on ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
        page,
      }) => {
        // Set viewport size
        ...
        expect(separatorBox!.x).toBeLessThanOrEqual(
          expectedLeftEdge + tolerance,
        );
        expect(separatorBox!.x + separatorBox!.width).toBeGreaterThanOrEqual(
          expectedRightEdge - tolerance,
        );
        ...
        expect(actualWidth).toBeGreaterThanOrEqual(
          expectedWidth * widthThreshold,
        );

        // Log for debugging
        console.log(
          `${viewport.name}: Board width: ${boardBox!.width}, Separator width: ${actualWidth}, Expected width: ${expectedWidth}`,
        );
      });
    }
  });
