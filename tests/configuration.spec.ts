...
import { DERIVED_CONFIG, SEMPOA_CONFIG } from '../src/config/sempoaConfig';
...
test.describe('Configuration Tests', () => {
  test.describe('Mathematical Formulas and Calculations', () => {
    test('derived config calculations should match expected mathematical formulas', async () => {
      ...
      console.log(
        `✓ SEPARATOR_TOP: ${DERIVED_CONFIG.SEPARATOR_TOP} = ${SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION} - (${SEMPOA_CONFIG.SEPARATOR.HEIGHT}/2)`,
      );
      ...
      console.log(
        `✓ SEPARATOR_BOTTOM: ${DERIVED_CONFIG.SEPARATOR_BOTTOM} = ${SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION} + (${SEMPOA_CONFIG.SEPARATOR.HEIGHT}/2)`,
      );

      // Test upper bead active position
      const expectedUpperActiveTop =
        SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION -
        SEMPOA_CONFIG.SEPARATOR.HEIGHT / 2 -
        SEMPOA_CONFIG.BEAD.HEIGHT;
      expect(DERIVED_CONFIG.UPPER_ACTIVE_TOP).toBe(expectedUpperActiveTop);
      console.log(
        `✓ UPPER_ACTIVE_TOP: ${DERIVED_CONFIG.UPPER_ACTIVE_TOP} = ${SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION} - (${SEMPOA_CONFIG.SEPARATOR.HEIGHT}/2) - ${SEMPOA_CONFIG.BEAD.HEIGHT}`,
      );

      // Test lower bead active position
      const expectedLowerActiveTop =
        SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION +
        SEMPOA_CONFIG.SEPARATOR.HEIGHT / 2 -
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT;
      expect(DERIVED_CONFIG.LOWER_ACTIVE_TOP).toBe(expectedLowerActiveTop);
      console.log(
        `✓ LOWER_ACTIVE_TOP: ${DERIVED_CONFIG.LOWER_ACTIVE_TOP} = ${SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION} + (${SEMPOA_CONFIG.SEPARATOR.HEIGHT}/2) - ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}`,
      );
      ...
      console.log(
        `✓ LOWER_INACTIVE_TOP: ${DERIVED_CONFIG.LOWER_INACTIVE_TOP} = ${SEMPOA_CONFIG.BEAD.HEIGHT}`,
      );
      ...
      console.log(
        `✓ LOWER_BEAD_SPACING: ${DERIVED_CONFIG.LOWER_BEAD_SPACING} = ${SEMPOA_CONFIG.BEAD.HEIGHT}`,
      );

      // Test main container and rod dimensions
      const expectedMainContainerHeight =
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT +
        SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT;
      expect(DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT).toBe(
        expectedMainContainerHeight,
      );
      console.log(
        `✓ MAIN_CONTAINER_HEIGHT: ${DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT} = ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT} + ${SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT}`,
      );

      const expectedRodHeight =
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT +
        SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT;
      expect(DERIVED_CONFIG.ROD_HEIGHT).toBe(expectedRodHeight);
      console.log(
        `✓ ROD_HEIGHT: ${DERIVED_CONFIG.ROD_HEIGHT} = ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT} + ${SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT}`,
      );

      // Test total board dimensions
      const expectedTotalBoardHeight =
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT +
        SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT;
      expect(DERIVED_CONFIG.TOTAL_BOARD_HEIGHT).toBe(expectedTotalBoardHeight);
      console.log(
        `✓ TOTAL_BOARD_HEIGHT: ${DERIVED_CONFIG.TOTAL_BOARD_HEIGHT} = ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT} + ${SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT}`,
      );

      const expectedTotalBoardWidth =
        SEMPOA_CONFIG.COLUMNS * SEMPOA_CONFIG.COLUMN.WIDTH;
      expect(DERIVED_CONFIG.TOTAL_BOARD_WIDTH).toBe(expectedTotalBoardWidth);
      console.log(
        `✓ TOTAL_BOARD_WIDTH: ${DERIVED_CONFIG.TOTAL_BOARD_WIDTH} = ${SEMPOA_CONFIG.COLUMNS} × ${SEMPOA_CONFIG.COLUMN.WIDTH}`,
      );
    });
    ...
    test('mathematical relationships should be logically consistent', async () => {
      ...
      expect(SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION).toBeGreaterThan(
        DERIVED_CONFIG.SEPARATOR_TOP,
      );
      expect(SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION).toBeLessThan(
        DERIVED_CONFIG.SEPARATOR_BOTTOM,
      );

      // Upper active position should be above upper inactive position
      expect(DERIVED_CONFIG.UPPER_ACTIVE_TOP).toBeGreaterThan(
        SEMPOA_CONFIG.POSITIONING.UPPER_INACTIVE_TOP,
      );

      // Rod should be tall enough to span both sections
      expect(DERIVED_CONFIG.ROD_HEIGHT).toBeGreaterThanOrEqual(
        DERIVED_CONFIG.TOTAL_BOARD_HEIGHT,
      );

      // Column width should be wider than bead width (for centering)
      expect(SEMPOA_CONFIG.COLUMN.WIDTH).toBeGreaterThan(
        SEMPOA_CONFIG.BEAD.WIDTH,
      );
  ...
  test.describe('Section Height Calculations', () => {
    test('section heights should be dynamically calculated from bead counts', async () => {
      ...
      const expectedUpperHeight =
        (SEMPOA_CONFIG.UPPER_BEADS_PER_COLUMN + 1) * SEMPOA_CONFIG.BEAD.HEIGHT;
      console.log(
        `Upper beads per column: ${SEMPOA_CONFIG.UPPER_BEADS_PER_COLUMN}`,
      );
      console.log(`Expected upper section height: ${expectedUpperHeight}px`);
      console.log(
        `Actual upper section height: ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}px`,
      );
      ...
      const expectedLowerHeight =
        (SEMPOA_CONFIG.LOWER_BEADS_PER_COLUMN + 1) * SEMPOA_CONFIG.BEAD.HEIGHT;
      console.log(
        `Lower beads per column: ${SEMPOA_CONFIG.LOWER_BEADS_PER_COLUMN}`,
      );
      console.log(`Expected lower section height: ${expectedLowerHeight}px`);
      console.log(
        `Actual lower section height: ${SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT}px`,
      );
    ...
    test('separator positioning should create no-gap, no-intersection layout', async () => {
      ...
      const upperBeadBottom =
        DERIVED_CONFIG.UPPER_ACTIVE_TOP + SEMPOA_CONFIG.BEAD.HEIGHT;
      expect(upperBeadBottom).toBe(DERIVED_CONFIG.SEPARATOR_TOP);
      console.log(
        `✓ Upper bead bottom (${upperBeadBottom}) touches separator top (${DERIVED_CONFIG.SEPARATOR_TOP})`,
      );

      // Lower bead (when active) top should touch separator bottom
      const lowerBeadTop =
        DERIVED_CONFIG.LOWER_ACTIVE_TOP + SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT;
      expect(lowerBeadTop).toBe(DERIVED_CONFIG.SEPARATOR_BOTTOM);
      console.log(
        `✓ Lower bead top (${lowerBeadTop}) touches separator bottom (${DERIVED_CONFIG.SEPARATOR_BOTTOM})`,
      );

      // Verify no overlap
      expect(upperBeadBottom).toBeLessThanOrEqual(DERIVED_CONFIG.SEPARATOR_TOP);
      expect(lowerBeadTop).toBeGreaterThanOrEqual(
        DERIVED_CONFIG.SEPARATOR_BOTTOM,
      );
      console.log(`✓ No overlap: upper bead ≤ separator ≤ lower bead`);
    });

    test('all beads should fit within their sections with proper spacing', async ({
      page,
    }) => {
      ...
      const requiredUpperSpace =
        upperBeadsCount * SEMPOA_CONFIG.BEAD.HEIGHT + SEMPOA_CONFIG.BEAD.HEIGHT;
      console.log(
        `Upper beads: ${upperBeadsCount}, Required space: ${requiredUpperSpace}px, Available: ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}px`,
      );
      ...
      const requiredLowerSpace =
        lowerBeadsCount * SEMPOA_CONFIG.BEAD.HEIGHT + SEMPOA_CONFIG.BEAD.HEIGHT;
      console.log(
        `Lower beads: ${lowerBeadsCount}, Required space: ${requiredLowerSpace}px, Available: ${SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT}px`,
      );
      ...
      const upperInactiveBottom =
        SEMPOA_CONFIG.POSITIONING.UPPER_INACTIVE_TOP +
        SEMPOA_CONFIG.BEAD.HEIGHT;
      expect(upperInactiveBottom).toBeLessThanOrEqual(
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT,
      );

      const upperActiveBottom =
        DERIVED_CONFIG.UPPER_ACTIVE_TOP + SEMPOA_CONFIG.BEAD.HEIGHT;
      expect(upperActiveBottom).toBeLessThanOrEqual(
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT,
      );

      console.log(
        `✓ Upper bead fits when inactive: ${upperInactiveBottom}px ≤ ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}px`,
      );
      console.log(
        `✓ Upper bead fits when active: ${upperActiveBottom}px ≤ ${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}px`,
      );
    });

    test('rod height should span full board container', async () => {
      // Rod height should equal main container height
      expect(DERIVED_CONFIG.ROD_HEIGHT).toBe(
        DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT,
      );

      // Rod height should equal calculated section heights (no extra space)
      const calculatedSectionTotal =
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT +
        SEMPOA_CONFIG.SECTIONS.LOWER_HEIGHT;
      ...
      console.log(
        `✓ Rod height (${DERIVED_CONFIG.ROD_HEIGHT}px) exactly matches container height`,
      );
      console.log(
        `✓ No extra space: container = sections = ${calculatedSectionTotal}px`,
      );
    });
  });
  ...
  test.describe('Dynamic Configuration Adaptation', () => {
    test('configuration should remain consistent for any bead count', async () => {
      ...
      expect(DERIVED_CONFIG.SEPARATOR_BOTTOM).toBe(
        calculations.separatorBottom,
      );
      ...
      expect(DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT).toBe(
        calculations.mainContainerHeight,
      );
      ...
      expect(SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION).toBe(
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT,
      );
      expect(DERIVED_CONFIG.LOWER_ACTIVE_TOP).toBeGreaterThanOrEqual(0);
    ...
    test('should demonstrate dynamic height adaptation examples', async () => {
      ...
      for (const example of examples) {
        ...
        console.log(
          `  Upper section: ${upperHeight}px (${example.upper} + 1) × ${BEAD_HEIGHT}px`,
        );
        console.log(
          `  Lower section: ${lowerHeight}px (${example.lower} + 1) × ${BEAD_HEIGHT}px`,
        );
    ...
    test('bead spacing should prevent intersections and allow proper stacking', async ({
      page,
    }) => {
      await page.goto(TEST_CONFIG.BASE_URL);
      ...
      console.log(
        `✓ Bead spacing (${DERIVED_CONFIG.LOWER_BEAD_SPACING}px) >= bead height (${expectedMinSpacing}px)`,
      );
      ...
      const bottomBeadPosition =
        DERIVED_CONFIG.LOWER_INACTIVE_TOP +
        3 * DERIVED_CONFIG.LOWER_BEAD_SPACING +
        SEMPOA_CONFIG.BEAD.HEIGHT;
      const bottomBeadPositionFromContainerTop =
        SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT + bottomBeadPosition;
      expect(bottomBeadPositionFromContainerTop).toBeLessThanOrEqual(
        DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT,
      );
      console.log(
        `✓ 4 beads fit in container: bottom bead at ${bottomBeadPositionFromContainerTop}px ≤ ${DERIVED_CONFIG.MAIN_CONTAINER_HEIGHT}px`,
      );

      // Test visual intersection by checking actual bead positions
      console.log(`\nTesting visual bead intersection...`);
      const lowerBeads = page
      ...
      for (let i = 0; i < beadCount - 1; i++) {
        ...
        if (currentBox && nextBox) {
          ...
          console.log(
            `Bead ${i} bottom: ${currentBottom}px, Bead ${i + 1} top: ${nextTop}px, Gap: ${visualGap}px`,
          );
