...
describe('SempoaBoard - Bead Positioning', () => {
  ...
  describe('Initial Bead Positioning', () => {
    test('should position upper beads at inactive positions initially', () => {
      ...
      upperBeads.forEach((bead, index) => {
        ...
        const expectedPosition = SEMPOA_CONFIG.POSITIONING.UPPER_INACTIVE_TOP + (index * SEMPOA_CONFIG.BEAD.HEIGHT);
    ...
    test('should position lower beads at inactive positions initially', () => {
      ...
      lowerBeads.forEach((bead, index) => {
        const beadContainer = bead.parentElement as HTMLElement;
        const expectedPosition = DERIVED_CONFIG.LOWER_INACTIVE_TOP + (index * DERIVED_CONFIG.LOWER_BEAD_SPACING);
        expect(beadContainer).toHaveStyle({
          top: `${expectedPosition}px`
        });
      });
  ...
  describe('Bead Movement and Positioning', () => {
    test('should move upper bead to active position when clicked', async () => {
      ...
      const expectedActivePosition = DERIVED_CONFIG.SEPARATOR_TOP - SEMPOA_CONFIG.BEAD.HEIGHT - 
        ((SEMPOA_CONFIG.UPPER_BEADS_PER_COLUMN - 1 - 0) * SEMPOA_CONFIG.BEAD.HEIGHT);
  ...
  describe('Column Structure and Layout', () => {
    ...
    test('should have proper section heights', () => {
      ...
      const upperSection = firstColumn.querySelector('.upper-section') as HTMLElement;
      expect(upperSection).toHaveStyle({
        height: `${SEMPOA_CONFIG.SECTIONS.UPPER_HEIGHT}px`
      });

      // Check lower section height
      const lowerSection = firstColumn.querySelector('.lower-section') as HTMLElement;
    ...
    test('should have main container with correct height', () => {
      ...
      const mainContainer = screen.getByTestId('sempoa-board').querySelector('.flex.justify-center.gap-2') as HTMLElement;
  ...
  describe('Rod and Separator Positioning', () => {
    test('should position vertical rods correctly', () => {
      ...
      const columns = sempoaBoard.querySelectorAll('[data-testid^="column-"]:not([data-testid*="header"])');
      const firstColumn = columns[0];
      const rod = firstColumn.querySelector('.bg-amber-900.rounded-full.shadow-sm') as HTMLElement;
    ...
    test('should position horizontal separator correctly', () => {
      ...
      const separator = screen.getByTestId('sempoa-board').querySelector('.bg-amber-900.rounded-full.shadow-md') as HTMLElement;

      // Check inline styles set by the component
      expect(separator.style.height).toBe(`${SEMPOA_CONFIG.SEPARATOR.HEIGHT}px`);
      expect(separator.style.width).toBe('100%');
      expect(separator.style.left).toBe('0px');
      expect(separator.style.top).toBe(`${SEMPOA_CONFIG.SEPARATOR.CENTER_POSITION}px`);
  ...
  describe('Mathematical Configuration Verification', () => {
    ...
    test('should verify all beads fit within their sections', () => {
      ...
      const requiredUpperSpace = SEMPOA_CONFIG.UPPER_BEADS_PER_COLUMN * SEMPOA_CONFIG.BEAD.HEIGHT;
      ...
      const requiredLowerSpace = SEMPOA_CONFIG.LOWER_BEADS_PER_COLUMN * SEMPOA_CONFIG.BEAD.HEIGHT;
